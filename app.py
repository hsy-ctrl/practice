...
@st.cache_data
def load_data():
    info = pd.read_csv("서울시_공영주차장_안내_정보.csv", encoding="euc-kr")
    rt   = pd.read_csv("서울시_시영주차장_실시간_주차대수_정보.csv", encoding="euc-kr")

    # 구 추출
    info["구"] = info["주소"].str.extract(r"([\w]+구)")
    rt["구"]   = rt["주소"].str.extract(r"([\w]+구)")

    # 이상치 제거 (총 주차면 10 이하) & 파생 컬럼
...
if "전체 현황" in view:
    ...
    with col1:
        ...
        fig = go.Figure(go.Bar(
            ...
            marker_color=[STATUS_COLOR[k] for k in cnt.index],
            marker_line_width=0,
    ...
    with col4:
        ...
        for nm, cnt in fi["운영구분명"].value_counts().head(5).items():
            pct = round(cnt / max(len(fi), 1) * 100, 1)
            st.markdown(f"**{nm}** `{cnt:,}개` {pct}%")
...
# ─────────────────────────────────────────
    ...
    with tab1:
        ...
        sort_opt = st.selectbox("정렬", ["이용률 높은 순","이용률 낮은 순","가용면 많은 순","가용면 적은 순"], key="s1")
        ...
        cols = ["구","주차장명","주소","총 주차면","현재 주차 차량수","가용면","이용률","혼잡도","기본 주차 요금","일 최대 요금"]
        disp_rt = fr2[cols].copy()
        disp_rt.columns = ["구","주차장명","주소","총 주차면","현재 차량","가용면","이용률(%)","혼잡도","기본요금(원)","일최대요금(원)"]
        ...
        st.download_button("📥 CSV 다운로드", disp_rt.to_csv(index=False, encoding="utf-8-sig"),
    ...
    with tab2:
        ...
        elif sort_opt2 == "기본요금 높은 순": fi2 = fi2.sort_values("기본 주차 요금", ascending=False)
        ...
        cols2 = ["구","주차장명","주소","주차장 종류명","운영구분명",
                 "총 주차면","유무료구분명","기본 주차 요금","기본 주차 시간(분 단위)","일 최대 요금","월 정기권 금액","전화번호"]
        disp_info = fi2[cols2].copy()
        disp_info.columns = ["구","주차장명","주소","종류","운영구분",
                              "총 주차면","유무료","기본요금(원)","기본시간(분)","일최대요금(원)","월정기권(원)","전화번호"]

        st.dataframe(
            disp_info.style.format({
                "총 주차면":"{:,.0f}","기본요금(원)":"{:,.0f}",
                "기본시간(분)":"{:,.0f}","일최대요금(원)":"{:,.0f}","월정기권(원)":"{:,.0f}",
            }),
            use_container_width=True, height=460, hide_index=True,
        )
        st.download_button("📥 CSV 다운로드", disp_info.to_csv(index=False, encoding="utf-8-sig"),
